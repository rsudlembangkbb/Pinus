...
import { validateAttendanceRow, ValidatedAttendanceRow } from '@/domain/import/validators';
...
export const POST = withApi(async (req: NextRequest) => {
  ...
  const r2Key = await storeRawFile(periodId, 'baraya_attendance', file);
  const batchId = await createImportBatch(db, { periodId, source: 'baraya_attendance', fileName: file.name, r2Key, uploadedBy: session.sub });
  ...
  rows.forEach((row, idx) => {
    const result = validateAttendanceRow(row, idx + 2, lookups);
  ...
  await writeAuditLog({
    ...
    entityId: batchId,
    after: { source: 'baraya_attendance', periodId, totalRows: rows.length, successRows: validRows.length, failedRows: errors.length }
  });

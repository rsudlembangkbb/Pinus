...
import { validateSimrsRow, ValidatedSimrsRow } from '@/domain/import/validators';
...
export const POST = withApi(async (req: NextRequest) => {
  ...
  if (!['draft', 'importing'].includes(period.status)) {
    return jsonError('Periode ini sudah melewati tahap impor data.', 409);
  }

  const buffer = await file.arrayBuffer();
  const r2Key = await storeRawFile(periodId, 'simrs', file);
  const batchId = await createImportBatch(db, { periodId, source: 'simrs', fileName: file.name, r2Key, uploadedBy: session.sub });

  const { rows } = parseWorkbookRows(buffer);
  const lookups = await buildLookups(db, 'simrs');

  const validRows: ValidatedSimrsRow[] = [];
  const errors: { rowNumber: number; column?: string; message: string }[] = [];

  rows.forEach((row, idx) => {
    const result = validateSimrsRow(row, idx + 2, lookups); // +2: header row + 1-indexed
  ...
  await insertRowsChunked(
    db,
    (r: ValidatedSimrsRow) =>
      db.insert(schema.serviceTransactions).values({
        id: newId('svc'),
        periodId,
        batchId,
        ...
        role: r.role,
        bpjsClaimNumber: r.bpjsClaimNumber
      }),
    validRows
  );
  ...
  await writeAuditLog({
    ...
    entityId: batchId,
    after: { source: 'simrs', periodId, totalRows: rows.length, successRows: validRows.length, failedRows: errors.length }
  });

  const unmatchedIdentities = validRows.filter((r) => !r.employeeId).length;
